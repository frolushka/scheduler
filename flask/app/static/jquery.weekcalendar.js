      ...
      */
      ...
      _setOption: function(key, value) {
         ...
         if(self.options[key] != value) {
            ...
            filtres = currentEvents.filtres;
            var newOptions = {};
            newOptions[key] = value;
            self._renderEvents({events:currentEvents.events, options: newOptions}, self.element.find(".wc-day-column-inner"))
        }
      ...
      _updateCheckbox : function() {
         ...
         for(var i = 0; i < filtres.length; ++i){
            $checkBox.append("<input type=\'checkbox\' class=\"boxFiltres\" value=\""+filtres[i]+"\" checked>"+filtres[i]+"<br>");
         }
      },
      ...
      _renderCalendar : function() {
         ...
         var $calendarContainer, calendarNavHtml, calendarHeaderHtml, calendarBodyHtml, $weekDayColumns, leftContainer;
         var self = this;
         var options = this.options;
         var $checkBox = $("#left").find('form[name=\'checkfiltr\']');
         ...
         options.filtres.splice(0,options.filtres.length);
         for(var i = 0; i < checked.length; ++i)
            options.filtres.push($(checked[i]).val());
         $weekDayColumns = self.element.find(".wc-day-column-inner");
         self._loadCalEvents(options.date);

         $checkBox.find(".boxFiltres").change(function() {
            var checked = $("input:checkbox:checked");
            options.filtres.splice(0,options.filtres.length);
            for(var i = 0; i < checked.length; ++i)
              options.filtres.push($(checked[i]).val());
      ...
      _addDroppableToWeekDay : function($weekDay) {
         ...
         $weekDay.droppable({
            ...
            drop: function(event, ui) {
               ...
               self._adjustForEventCollisions($weekDay, $calEvent, calEvent, calEvent, true);
      ...
      _addResizableToCalEvent : function(calEvent, $calEvent, $weekDay) {
         ...
         $calEvent.resizable({
            ...
            stop :function(event, ui) {
               ...
               options.eventResize(calEvent, calEvent, $calEvent);
               $calEvent.data("preventClick", true);
               setTimeout(function() {
